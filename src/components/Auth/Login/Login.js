...
import {Card, Container, InputAdornment, Typography} from '@mui/material';
    ...
    <div className="background">
      <Container className="container-login" align={'center'}>
        <Card className="box-shadow"
              sx={{borderRadius: 0, pt: 5, pb: 5, pl: 2, pr: 2, backgroundColor: 'rgba(48, 47, 47, 0.3)'}}>
          <Formik initialValues={{email: '', password: ''}}
            ...
            <Form>
              ...
              <Typography className='hand-cursor' onClick={()=> navigate('/sign-up')}
                sx={{color: '#fff', fontSize: 14, mb: 1.5, mt: 0.5, cursor: 'pointer'}} align={'left'}
              >
                Sign Up
              </Typography>
              <div style={{display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: 15}}
                ...
                <Typography sx={{color: '#fff', fontSize: 14, ml: 1, cursor: 'pointer'}}
                            onClick={() => navigate(-1)}>back to home</Typography>
              </div>
            </Form>
          </Formik>
        </Card>

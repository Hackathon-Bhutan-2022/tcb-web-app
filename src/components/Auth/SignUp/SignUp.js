  ...
  const onSubmit = (values, {setSubmitting}) => {
    ...
    if (roleId === 'agent') {
      usersServices('post', null, {
        user: {
          ...values,
          tour_agency_profile_attributes: {...values?.profile}
        }
      }).then(response => {
        navigate('/login');
        dispatch({
          type: SNACKBAR_OPEN,
          payload: {isNotify: true, severity: 'success', message: 'Agent registered successfully'}
        });
      });
    } else {
      usersServices('post', null, {
        user: {
          ...values,
          tour_guide_profile_attributes: {...values?.profile}
        }
      }).then(response => {
        navigate('/login');
        dispatch({
          type: SNACKBAR_OPEN,
          payload: {isNotify: true, severity: 'success', message: 'User registered successfully'}
        });
      });
    }
    setSubmitting(false);

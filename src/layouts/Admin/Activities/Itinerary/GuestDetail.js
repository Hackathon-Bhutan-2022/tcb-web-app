...
export default function GuestDetail() {
  ...
  const handleAdd = () => {
    setFieldValue(`itinerary_guests_attributes`, [
      ...values?.itinerary_guests_attributes,
      {name: '', age: null, nationality: ''}
    ]);
  };
  ...
  return (<Grid container spacing={2}>
    ...
    <Grid item lg={6} md={6} sm={6} xs={12}>
      {values?.itinerary_guests_attributes?.map((guest, index) => (<Accordion key={index}>
        <AccordionSummary
